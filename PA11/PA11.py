...
def optimize_hyperspace_routes(star_list,jump_times):
    ...
    for k in range (0, n):
        for i in range (0, n):
            for j in range (0, n):
                thru_k = jump_times[i][k] + jump_times[k][j]
                if thru_k < jump_times[i][j]:
                    jump_times[i][j] = thru_k
                    Prev[i][j] = Prev[k][j]

    if not detect_negative(jump_times):
        return jump_times
    else:
...
def detect_negative(Dist):
    for i in range (0, len(Dist)):
        if Dist[i][i] < 0:
            return True
    return False

def HandleNegativeCycle(P, D, star_class):
    ListOfNegative = []
    ListOfDistance = []
    for i in range (0, len(P)):
        ListOfNegative.append(P[i][i])
        ListOfDistance.append(D[i][i])

    ReturnList = []
    ReturnList.insert(0, star_class[len(ListOfNegative)-1])
    old = len(ListOfNegative)-1
    new = None

    while new != len(ListOfNegative)-1:
        new = ListOfNegative[old]
        ReturnList.insert(0,star_class[new])
        old = new
