...
def CountingSort(studentList,houseOrYear):
    ...
    # make answer where you print out the sorted array
    for index1 in range (0, len(studentList)):
        answer.append(0)
    # make count where it stores the value
    for index2 in range (0, k+1):
        count.append(0)

    # count array !!
    for index3 in range (0, len(studentList)):
        count[studentList[index3].getIndex(houseOrYear)] += 1
    # finalizing array count
    ...
    for index in range (len(studentList)-1, -1, -1):
        answer[count[studentList[index].getIndex(houseOrYear)]] = studentList[index]
        count[studentList[index].getIndex(houseOrYear)] -= 1
...
eg = getStudentList('roster0.csv')[0]
print(eg.getIndex('h'))
