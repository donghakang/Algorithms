...
def optimizeNuggets(priceList,numNuggets):
    ...
    # at this point we have rev and split

    for index1 in range (1, numNuggets + 1):
        FindCheapest = 10000
        for index2 in range (1, index1 + 1):
            if FindCheapest > priceList[index2] + rev[index1-index2]:
                FindCheapest = priceList[index2] + rev[index1-index2]
                split[index1] = index2
        rev[index1] = FindCheapest

    # Now we have array that represents revenue and split.
    # Now make a list of integers, that shows the number of chicken nuggets

    ChickenNuggets = numNuggets
    correct = []
    # correct is the list that will be returned
